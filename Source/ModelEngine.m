...
- (LanguageModel*)createLanguageModelForLanguage:(NSString*)language identical:(BOOL)identical layout:(BOOL)layout copyOnlyIfExists:(BOOL)copyOnlyIfExists source:(BundleSource*)source
{
    ...
    // todo
    for(NSString *path in @[]) { //[source sourcePaths]) {
...
- (FileModel*)createFileModelForLanguage:(NSString*)language sourceFileModel:(FileModel*)sourceFileModel copyOnlyIfExists:(BOOL)copyOnlyIfExists translateFromSourcePath:(NSString*)sourcePath
{
	NSString *sourceRelativePath = [sourceFileModel relativeFilePath];
	NSString *sourceAbsolutePath = [[self projectModel] absoluteProjectPathFromRelativePath:sourceRelativePath];
...
- (FileModel*)createFileModelForLanguage:(NSString*)language sourceFileModel:(FileModel*)sourceFileModel layout:(BOOL)layout copyOnlyIfExists:(BOOL)copyOnlyIfExists updateFromSourcePath:(NSString*)sourcePath
{
    ...
    if((copyOnlyIfExists && [localizedSourceFile isPathExisting]) || !copyOnlyIfExists) {
        ...
        if([engine supportsContentTranslation]) {
            ...
            if([ImportFilesConflict resolveConflictBetweenProjectFile:targetAbsolutePath andImportedFile:localizedSourceFile provider:[self projectProvider]] == RESOLVE_USE_IMPORTED_FILE) {
                [[self console] addLog:[NSString stringWithFormat:@"Replace non-translatable file \"%@\" by \"%@\"", targetAbsolutePath, localizedSourceFile] class:[self class]];
                [[FileTool shared] copySourceFile:localizedSourceFile
                                    toReplaceFile:targetAbsolutePath
                                          console:[self console]];				
            }
	...
	if([localizedSourceFile isPathExisting] && [[self fileModuleEngineForFile:targetAbsolutePath] supportsContentTranslation] && [localizedSourceFile isEqualToString:targetAbsolutePath] == NO) {
